MasterSwiscoin-qt: Qt4 GUI for MasterSwiscoin
===============================
...
Alternatively, install Qt Creator and open the `MasterSwiscoin-qt.pro` file.

An executable named `MasterSwiscoin-qt` will be built.
...
A warning for people using the *static binary* version of MasterSwiscoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of MasterSwiscoin is linked against libdb5.3.
...
installed causes MasterSwiscoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
