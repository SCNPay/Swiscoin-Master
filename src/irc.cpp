...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("MasterSwiscoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MasterSwiscoinTEST2\r");
            Send(hSocket, "WHO #MasterSwiscoinTEST2\r");
        } else {
            // randomly join #MasterSwiscoin00-#MasterSwiscoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MasterSwiscoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MasterSwiscoin%02d\r", channel_number).c_str());
        }
